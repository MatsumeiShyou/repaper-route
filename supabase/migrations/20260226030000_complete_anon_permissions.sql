...
BEGIN
    ...
    -- 6. New Features (e.g., manual_injection_reasons)
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'manual_injection_reasons') THEN
        GRANT ALL ON TABLE public.manual_injection_reasons TO anon;
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'manual_injection_reasons' AND policyname = 'Enable all access for anon') THEN
            CREATE POLICY "Enable all access for anon" ON public.manual_injection_reasons FOR ALL TO anon USING (true) WITH CHECK (true);
        END IF;
    END IF;
