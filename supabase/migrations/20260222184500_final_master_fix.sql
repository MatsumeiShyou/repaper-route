-- 20260222184500_final_master_fix.sql
-- 物理スキーマの正典化と RPC (SDR執行器) の堅牢化

-- [1] 物理カラムの追加
ALTER TABLE public.drivers ADD COLUMN IF NOT EXISTS is_active boolean DEFAULT true NOT NULL;
ALTER TABLE public.master_contractors ADD COLUMN IF NOT EXISTS is_active boolean DEFAULT true NOT NULL;
...
BEGIN
    ...
    IF p_table_name = 'vehicles' THEN
        ...
        VALUES (
            v_target_id_uuid,
            COALESCE(p_core_data->>'number', 'NEW'),
    ...
    ELSIF p_table_name = 'items' THEN
        ...
        VALUES (
            v_target_id_uuid,
            COALESCE(p_core_data->>'name', 'NEW'),
            COALESCE(p_core_data->>'unit', '袋'),
    ...
    ELSIF p_table_name = 'points' OR p_table_name = 'master_collection_points' THEN
        INSERT INTO public.master_collection_points (
            ...
            entry_instruction, safety_note, site_contact_phone, note, is_active, updated_at
        )
        VALUES (
            v_target_id_text,
            COALESCE(p_core_data->>'name', p_core_data->>'display_name', 'NEW'),
        ...
        ON CONFLICT (location_id) DO UPDATE SET
            name = COALESCE(p_core_data->>'name', EXCLUDED.name, public.master_collection_points.name),
            ...
            contractor_id = COALESCE(p_core_data->>'contractor_id', public.master_collection_points.contractor_id),
            visit_slot = COALESCE(p_core_data->>'visit_slot', public.master_collection_points.visit_slot),
    ...
    ELSIF p_table_name = 'drivers' THEN
        -- 堅牢化: アーカイブ時は既存レコードを優先更新。新規の場合は最低限の値を投入。
        INSERT INTO public.drivers (id, driver_name, display_order, user_id, is_active, updated_at)
        VALUES (
            v_target_id_text,
            COALESCE(p_core_data->>'name', p_core_data->>'driver_name', 'NEW'),
        ...
        ON CONFLICT (id) DO UPDATE SET
            ...
            display_order = COALESCE((p_core_data->>'display_order')::integer, public.drivers.display_order),
            user_id = COALESCE(p_core_data->>'user_id', public.drivers.user_id),
    ...
    ELSIF p_table_name = 'users' THEN
        ...
        VALUES (
            v_target_id_text,
            COALESCE(p_core_data->>'name', 'NEW'),
            COALESCE(p_core_data->>'role', 'driver'),
        ...
        ON CONFLICT (id) DO UPDATE SET
            ...
            vehicle_info = COALESCE(p_core_data->>'vehicle_info', public.profiles.vehicle_info),
            updated_at = NOW();
    ...
    ELSIF p_table_name = 'contractors' OR p_table_name = 'master_contractors' THEN
        ...
        ON CONFLICT (contractor_id) DO UPDATE SET
            ...
            payee_id = COALESCE(p_core_data->>'payee_id', public.master_contractors.payee_id),
