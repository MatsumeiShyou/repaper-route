-- 20260306231747_update_rpc_board_snapshot.sql
-- Phase: 12 (Data Governance)
-- Overview: 二段階確定モデル対応。BULK_CONFIRM時にスナップショットを自動保存するようにrpc_execute_board_updateを更新
...
BEGIN
    ...
    VALUES (
        ...
        NOW(),
        v_current_user_id,
        CASE WHEN p_decision_type = 'BULK_CONFIRM' THEN p_new_state ELSE NULL END,
        CASE WHEN p_decision_type = 'BULK_CONFIRM' THEN NOW() ELSE NULL END
    )
    ON CONFLICT (date) DO UPDATE SET
        ...
        confirmed_snapshot = CASE 
            WHEN p_decision_type = 'BULK_CONFIRM' THEN p_new_state 
            ELSE public.routes.confirmed_snapshot 
        END,
        confirmed_at = CASE 
            WHEN p_decision_type = 'BULK_CONFIRM' THEN NOW() 
            ELSE public.routes.confirmed_at 
        END;
