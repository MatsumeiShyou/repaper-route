-- 20260222190000_perfect_sdr_rpc.sql
-- SDR執行権限の厳格化と論理バリデーションの統合
...
BEGIN
    ...
    -- [2] Logical Validation (Phase 5-2: ENTRY_CREATE vs ATTRIBUTE_UPDATE)
    IF v_target_id_text IS NULL THEN
        -- 新規作成 (CREATE) のバリデーション
        IF v_is_archive THEN
            RAISE EXCEPTION 'Cannot create an archived record.';
        END IF;

        -- 必須項目チェック (テーブル毎に物理制約の前に論理チェック)
    ...
    -- [4] Final Execution (Perfect Upsert Mapping)
    ...
    ELSIF p_table_name = 'points' OR p_table_name = 'master_collection_points' THEN
        INSERT INTO public.master_collection_points (
            ...
            vehicle_restriction_type, restricted_vehicle_id, target_item_category, is_active, updated_at
        )
        VALUES (v_target_id_text, COALESCE(p_core_data->>'name', p_core_data->>'display_name'), p_core_data->>'display_name', p_core_data->>'address', p_core_data->>'contractor_id', COALESCE(p_core_data->>'visit_slot', 'FREE'), COALESCE(p_core_data->>'vehicle_restriction_type', 'NONE'), CASE WHEN p_core_data->>'restricted_vehicle_id' IS NULL OR p_core_data->>'restricted_vehicle_id' = '' THEN NULL ELSE (p_core_data->>'restricted_vehicle_id')::uuid END, p_core_data->>'target_item_category', COALESCE((p_core_data->>'is_active')::boolean, true), NOW()
        )
